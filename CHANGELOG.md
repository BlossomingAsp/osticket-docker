...
- **Discord bot 2-way sync**: on top of the base bot, tickets created on osTicket are mirrored into the Discord channel (with a per-ticket thread), staff/client replies are posted into the thread, and the reaction on the original message tracks the current status. A reaction on a message changes the ticket status back in osTicket. Backed by `discord_thread_id`/`last_thread_entry_id` columns and controlled by `OSTICKET_DISCORD_THREAD_ENABLED`/`OSTICKET_DISCORD_THREAD_PREFIX`.
- **Discord bot assignee notifications** (`OSTICKET_DISCORD_STAFF_MATCH=1`): the bot LEFT JOINs `ost_staff` onto the ticket to get the assigned staff member's `username`, matches it exactly (case-insensitive) against Discord usernames (`user.name`, incl. any `_1234` suffix), and mentions the assigned member in the thread when a **client** replies. Requires the Server Members Intent. Staff replies are never assignee-mentioned.
- **Optional self-hosted mail server (Stalwart)**: a single-container `mail` service (SMTP + IMAP/JMAP + built-in anti-spam/DKIM/SPF/DMARC + Let's Encrypt) plus a `cron` service that polls `api/cron.php` so osTicket delivers queued mail and fetches inbound IMAP replies. Both are gated behind compose `profiles: ["mail"]`; when `OSTICKET_MAIL_ENABLED=1` is set, `install.sh`/`update.sh` pass `--profile mail`. Egress is deliberately IPv4-only (single-IPv4-PTR providers), and the docs capture the PTR / port-25 / DNS-only-A delivery checklist. osTicket's SMTP (587) and IMAP (993) point at host `mail`; first-run domain + mailbox setup happens in the Stalwart admin UI.

### Fixed

- **Interactive one-liner via process substitution**: the one-liner is now `bash <(curl -sSL .../get-osticket.sh)` — process substitution keeps stdin connected to the terminal, so `install.sh` genuinely prompts for passwords, port, and helpdesk details (piped `curl ... | sh` consumes stdin and can't prompt). Piped installs are deliberately rejected with a pointer to the correct command: `get-osticket.sh` refuses up front, and `install.sh` dies unless `-y` (non-interactive) is given. The previous `/dev/tty` redirect approach was dropped as unreliable (it failed to display prompts over SSH).
...
- **OAuth instance config is declarative**: `docker/provision.php` previously created each auth-oauth2 instance once and left it untouched afterwards — so a `redirectUri` baked from the wrong helpdesk URL (e.g. `http://localhost/api/auth/oauth2` captured during auto-install) never corrected itself when `OSTICKET_HELPDESK_URL` was later set, and Discord rejected the callback with "redirect_uri: Not a well formed URL". `create_instance()` now calls `update_instance_config()` on an existing instance, reconciling the plain config fields (`redirectUri`, `clientId`, provider endpoints, scopes, attribute mappings) with the env-derived values on every start. Secrets (`clientSecret`, a `PasswordField` holding ciphertext) and `SectionBreakField` separators are left alone.
- **First-start provisioning no longer crashes**: on the very first `docker compose up` after install, `ensure_plugin()` used the base `Plugin` object returned by `PluginManager::install()` (`Plugin::create`), whose `config_class` is null — so `addInstance()` failed with "Call to a member function getForm() on null" (exit 255) and the container died once before auto-restarting. `ensure_plugin()` now returns the plugin impl subclass (like `PluginManager::lookup()` does), re-asserting `isactive` on it because the impl's `lookup()` can hand back a stale ORM row right after install. Clean first start: zero restarts.
- **DB readiness healthcheck**: the `db` healthcheck used `mariadb-admin ping`, which reports alive before MariaDB actually accepts queries — so `osticket` could start before the DB was truly ready and the auto-install wizard failed. The healthcheck now runs `mariadb ... -e "SELECT 1"`, which only passes once the server accepts real queries.
